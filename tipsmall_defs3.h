...
struct gassmall{
  ...
  float mass; 
  float fmassloss;
  int grpid; //added for halo tracking
  int snap;
};
...
struct gasout{
  int iord;
  int id; 
  float eu_cumu; //eu became eu_cumu
  float eu_curr; //current eu an this step
  float eu_sum; //eu_curr + previous eu_curr
  float mass;
  float fmassloss;
  int grpid; //added for halo tracking
  float dwarfrac;
