...
int main(int argc, char *argv[]){
  ...
  struct starlog *slog;
  int i,j,k,nord, nfiles=400, norigin, nslog, origin=15523200;
...
/*---------------------------------------------------------------------------------*/
 /*                                     BLOCK3                                   */
  ...
  for(i=0;i<=nfiles-1;i++){
    ...
    sprintf(filename3, "/mn/stornext/d17/extragalactic/personal/gabrierg/rprocess_halos/L90Mpc8000_hithres.%05d.gord_euonly_cumu_idx1",i);
	...
	gprev[k].grpid = -1; //added to output to track halo id /*//////////////new addition//////////////*/
      }
    } else {
...
/*---------------------------------------------------------------------------------*/
 /*                                     BLOCK4                                   */
    ...
    for(k=0;k<=nuniq-1;k++){
      ...
      gout[k].grpid = gord[uniq[k]].grpid; //if uniq is made 2D then call 2nd col and have the prev gout's call 1st col /*/////////////new addition/////////////*/
    }

    /*TEST PRINTS*/
    //for(i=0; i<25; i++){
    //printf("%d %d %g %g %g \n", gout[k].iord,gout[k].id,gout[k].eu,gout[k].mass,gout[k].fmassloss);
    //}

    //for(k=2583158; k<=2583178; k++){
    //printf("%d %d %g %g %d \n", gout[k].iord, gout[k].id, gout[k].eu, gout[k].mass);//, gout[k].grpid);
    //}
