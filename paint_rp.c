...
int main(int argc, char *argv[]){
  ...
  fptipsy = fopen(argv[1], "r"); //reads in L90Mpc8000_hithres sim files
  assert(fptipsy != NULL); 
  Tipsy_in(fptipsy, &parts); 
  ng = parts.h.nsph;            //number of gass particles
  nd = parts.h.ndark;           //number of dark matter particles
  fclose(fptipsy); 
  printf("Number of Gas particles:  %d \n", ng);
  printf("total number of particles %d \n", ng+nd+parts.h.nstar);

  z = 1./parts.h.time -1; //redshift
  printf("z  %f \n", z); 
  /* list of stellar particles randomly chosen to paint its neighboring gas with Eu */

  fplist = fopen(argv[2], "r"); //reads in output files from step0 .sid_merger_0p05
  assert(fplist !=NULL); 
  fscanf(fplist, "%d \n", &npick); //npick = total number of random of stars picked
  ...
  fpout=fopen(argv[3], "w"); //output file for this step .painteu.gidlist_idx1p0
  assert(fpout != NULL);
  fprintf(fpout, "%d \n", npick*nsm);
  //fclose(fpout); 
  // dist = (float *)malloc(ng*sizeof(float));

  fplog = fopen(argv[4], "w"); //log output for this step .painteu.gidlist_idx1p0.log
  assert(fplog !=NULL); 
  
  for (i=0;i<=npick-1;i++){
    fprintf(fplog,"stellar particle ID %d \n", ipick[i]);
    xc = parts.sp[ipick[i]].pos[0]; //center x coord
    yc = parts.sp[ipick[i]].pos[1]; //center y coord
    zc = parts.sp[ipick[i]].pos[2]; //center z coord
    search_r = 2.0;                 //some search parameter
    //   count = 0; 
    for(l=0;l<=6;l++){
	count = 0;
	k = 0;
	for (j=0;j<=ng-1;j++){
	  //calc the dist between gas particle from star particle for all gas partiles
	  ...
	  if(dist*kpcunit/(1+z) <= search_r){/* if the dist is smaller than search parameter count it as enrichment event*/
	    count = count+1; 
	  }
	}
	if (count >=nsm) break; 
    ...
    for(j=0;j<=ng-1;j++){
      ...
      if(dist*kpcunit/(1+z) <= search_r){//sort gas info
	gas[k].dist=dist;
    ...
    mtot = 0.0; 
    for(k=0;k<=nsm-1;k++){//total mass for all smoothed particles
      mtot = mtot+gas[k].mass*kernel3D(gas[k].dist, hsm); 
    }
    for(k=0;k<=nsm-1;k++){//amount of eu for all smoothed particles
      gas[k].eu = gas[k].mass*kernel3D(gas[k].dist, hsm)/mtot*Eutot;
    }
    //fpout=fopen(argv[3], "a"); 
    //assert(fpout != NULL); 
    for(k=0;k<=nsm-1; k++){//print gas id, stellar id, gas dist, gas eu
