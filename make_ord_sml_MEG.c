...
int main(int argc, char *argv[]){
  ...
  struct starlog *slog;
  XDR xdrs;
  int i, j, ng, nfiles=400, nd, norigin, nwrite, ntotgrp, ntot, ntoteu, nstart, nend, ns, nslog, nread, n, snap;
  ...
  //char filename1[88] = argv[1];/mn/stornext/u3/shens/scratch/Eris_data/L90Mpc8000_hithres.00000
  //printf("file name:%s\n",argv[1]);//filename1);
  //printf("snap:%d\n",atoi(argv[1] + 61));
  snap = atoi(argv[1] + 61);
...
/*                                BLOCK2                              */
  ...
  gout = (struct gasout *)malloc(norigin *sizeof(struct gasout)); 
  for (i=0; i<=norigin-1; i++){
    gout[i].iord = i;
    gout[i].id = -1; 
    gout[i].eu_curr = 0;       //mass of Europium received in the current timestep
    gout[i].eu_cumu = 0;
    gout[i].eu_sum = 0;
    gout[i].mass = 0; 
    gout[i].fmassloss = 0;
    ...
    gout[i].dwarfrac = 0;
    gout[i].snap = snap;  //snapshot number
  }
...
/*                                BLOCK3                              */
  for(j=0;j<=ntoteu-1;j++){
    gout[iord[ideu[j]]].eu_curr = gout[iord[ideu[j]]].eu_curr + masseu[j]; /*in case the same gas particle receive twice of r-process within a single timestep*/
  }
  ...
  for(i=0;i<=norigin-1;i++){    
    ...
    //printf("%d %g %g %d\n", gout[i].id, gout[i].mass, gout[i].eu_curr, gout[i].grpid);
    printf("%d %g %g %d \n", i, gout[i].mass, gout[i].eu_curr, gout[i].grpid);
  }
 
  for(i=0; i<=20; i++){
    //printf("first 20 data \n");
    //printf("%d, %g, %g, %g, %d, %d \n", gout[i].id, gout[i].eu_curr, gout[i].mass, gout[i].fmassloss, gout[i].grpid, gout[i].snap); //added gout[i].grpid as 5th output 
  }
  
  for(i=norigin-1; i>=norigin-21; i--){
    //printf("last 20 data \n");
    //printf("%d, %g, %g, %g, %d \n", gout[i].id, gout[i].eu_curr, gout[i].mass, gout[i].fmassloss, gout[i].grpid); //added gout[i].grpid as 5th output 
  }
...
/*                                BLOCK4                              */
  ...
  //  fprintf(fpout, "%d \n", norigin); 
  nwrite = fwrite(gout, sizeof(struct gasout), norigin, fpout); 
  assert(nwrite == norigin); 
  //for (i=0; i<=norigin-1; i++){
    //  fwrite(gout[i], sizeof(struct gassmall), 1, fpout); 
    //fprintf(fpout, "%d %g %g %d \n", i, gout[i].mass, gout[i].eu_curr, gout[i].grpid); 
  //} 
  fclose(fpout); 
